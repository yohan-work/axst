...
**대상**: 진단 리포트에서 **검증·신뢰 조정**이 🟠 보완 필요로 나온 사람, 또는 주관식 검증 설계가 3점 이하인 사람. ⚪ 판정 보류는 처방 대상이 아니지만([docs/06](../docs/06-prescription.md)) 원하면 풀어도 된다.
**소요**: 약 50분 (혼자) · 60분 (팀 세션, [진행안](README.md#팀-세션-진행안-60분))
...
| 질문 | '예'면 |
|---|---|
| 이 주장이 틀리면 **결정이 바뀌는가**? (머지·배포·공지) | 앞 순서로 |
| **가진 자료로 바로 확인**할 수 있는가? (diff·로그·원문) | 앞 순서로 |

두 질문의 답이 비슷하면 **"없다"·"전체"·"문제없다"는 주장을 먼저** 본다. 있다고 한 것은 눈에 보이지만, 없다고 한 것은 찾아보지 않으면 드러나지 않는다.

한 문장에 **확인되는 부분과 자료가 없는 부분이 섞여 있으면 나눠서** 판정한다.

### 따로 다루는 두 종류

- **했다고 하는데, 그럴 수단도 수행 기록도 자료에 없는 주장** — "브라우저에서 동작 확인했습니다", "배포 후 모니터링 결과 이상 없습니다". 만든 쪽이 무엇에 접근할 수 있었는지부터 본다. 수단도 기록도 없으면 이 문장은 검증할 대상이 아니라 **근거 없음**이다. 지우고, 사람이 할 일로 넘긴다.
...
## 2. 실습 1 — 배포 전 PR 요약 (25분)

### 상황

금요일 오후, 상품 상세 페이지 배포를 앞두고 있다. 당신은 옵션 선택 드롭다운을 리팩터링한 PR을 올렸다. 팀에서 쓰는 AI 코드 리뷰 도우미가 **PR 설명과 확인 결과 요약**을 자동으로 써 줬다. **이 도우미는 PR의 diff와 당신이 붙여 넣은 로컬 명령 출력(자료 2)만 입력으로 받는다.** 브라우저나 배포 환경에는 접근하지 못한다.

**팀장은 이 요약만 보고 머지와 배포를 승인한다.** 이 레포는 PR 단계에 CI가 없고, 빌드와 전체 테스트는 배포 파이프라인에서만 돈다. 파이프라인에서 깨지면 금요일 배포가 그대로 멈춘다.
...
-  const [opt, setOpt] = useState(names[0]);
...
### 자료 3 — `OptionSelect`를 쓰는 곳 (import 줄 등 일부 생략)
...
**A. 확인 순서** — ①~⑧ 중 **확인할 것**만 먼저 볼 순서대로 적는다. 각각 어느 자료로 확인하는지도 적는다. **칸을 다 채울 필요는 없다.**
...
> LCP 이미지도 `next/image` 기본 설정 그대로 두면 됩니다. 사내 성능 가이드 v2 **3.2절**에 "기본값을 유지한다"고 되어 있습니다.
...
위키에 올리기 전에 무엇을 하겠는지, 그리고 위키에 무엇을 적겠는지 **세 줄**로 적는다.
...
| 주장 | 틀리면 결정이 바뀌나 | 가진 자료로 바로 확인되나 | "없다·전체·문제없다" 주장인가 | 수단·기록 없이 "했다"는 주장인가 |
|---|---|---|---|---|
| | | | | |
| | | | | |
| | | | | |
...
- 어떻게 확인하나 (원자료와 대조 / 다시 계산 / 독립적으로 다시 만들어 비교 / 동료 리뷰): ______________________________________________
...
| 틀렸을 때 **할 일**을 적었다 | ☐ |
| **누가·언제** 확인하는지 적었다 | ☐ |
| **먼저 볼 것의 순서와 보지 않을 것**을 정했고, 이유가 있다 | ☐ |
