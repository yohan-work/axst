...
> [!WARNING]
> 채점 리허설은 앵커를 쓴 모델과 같은 계열이 채점했으므로 **독립 검증이 아닙니다.** 일치는 타당도의 증거가 아니고 불일치만 신호로 봅니다. 알려진 주요 문제 — 페이킹 방어 상한의 발동 불안정, FIN 페르소나 감점 편향, 주관식 질문이 D3 만점 요건을 노출하는 문제 — 는 [열린 과제](docs/12-open-issues.md)에 정리했습니다.
...
막지 않고 쓸모없게 만듭니다. AI 사용을 허용하되 사용한 프롬프트를 첨부하게 하고, 그것도 채점 자료로 씁니다. 실제 업무명·시스템명·데이터 항목명이나 페르소나 지문의 조건이 없는 범용 답안은 일부 차원이 상한에 갇힙니다. 다만 이 방어가 아직 불안정하다는 것을 리허설에서 확인했고, [열린 과제](docs/12-open-issues.md)의 최우선 항목입니다.
</details>
